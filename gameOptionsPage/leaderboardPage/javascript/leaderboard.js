...
function sortGeodashData(snapshot) {
    ...
    for (let i = 0; i < Math.min(grab_Userdata.length, 5); i++) {
        geodash_leaderboardData_display += grab_Userdata[i][0] + " Highscore: " + grab_Userdata[i][1].Highscore + " <br> "
    }
}
function sortJetpackmanData(jetpackmanData) {
    ...
    for (let i = 0; i < Math.min(jetpackman_grab_Userdata.length, 5); i++) {
        jetpackman_leaderboardData_display += jetpackman_grab_Userdata[i][0] + " Highscore: " + jetpackman_grab_Userdata[i][1].Highscore + " <br> "
    }
